...
//     const isOwnMessage = message.senderId === 'PAGE_ID';
...
const MessagePanel = ({ messages =[], onSendMessage, customerName }) => {
    ...
    return (
        <div className="w-full lg:w-2/3 bg-white flex flex-col">
            <div className="bg-gray-100 p-4 text-lg font-semibold border-b border-gray-200">
                Conversation with {customerName}
            </div>

            <div className="border-b border-gray-200 p-4 text-lg font-semibold">
                {/* Removed the duplicate customer name display */}
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages && messages.map((message, index) => (
                    <Message key={index} message={message} />
